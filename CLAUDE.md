...
- **푸시 전에 `npm run build` 를 돌린다.** vitest 는 esbuild 라 타입을 보지 않는다 —
  테스트가 다 통과해도 `tsc` 에서 깨질 수 있고, 그러면 Vercel 배포가 실패한다.
  (실제로 한 번 깨뜨렸다: `noUncheckedIndexedAccess` 가 인덱싱에 `| undefined` 를 붙인다.)
