...
export class Playtest {
  ...
  /**
   * 결과 보내기 버튼을 드러낸다. 카드를 열지는 않는다.
   *
   * **잔기를 다 썼을 때 부른다.** 게이트 난이도(기사)는 잔기가 3이라 네 번째
   * 사망에서 게임 오버가 뜨는데, 버튼은 다섯 번째 사망에야 나온다. 테스터가
   * 가장 그만두기 쉬운 순간에 결과를 보낼 방법이 없다는 뜻이다.
   *
   * 여기서 카드를 **열지는 않는다** — GAME OVER 화면을 덮으면 이어하기를
   * 누를 수가 없다. 버튼만 세워 두고 선택은 테스터가 한다.
   */
  revealEntry(): void {
